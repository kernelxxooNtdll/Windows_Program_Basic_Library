	...
	)
{
	...
	if (wcsstr(szParentProcess, L"\\explorer.exe") == NULL && wcsstr(szParentProcess, L"\\kslaunch.exe") == NULL)
		return;
	...
	//
	if (LhpIsWow64Process(szCurrentProcess))
	{
		LhpLockCommandLineWow64(ProcessId, szRing3ProcessName);
	}
	else
	{
    ...
    )
{
	...
	if (wcsstr(szModuleName, L"kernelbase.dll") == NULL)
		return;
